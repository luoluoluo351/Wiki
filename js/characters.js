...
function calcCombatPower(c) {
  let pts = 0;
  pts += (c.basicAttr.hp.lv100 || 0) / 10;
  pts += (c.basicAttr.atk.lv100 || 0) / 2;
  pts += (c.basicAttr.def.lv100 || 0) / 2;
  pts += (c.advancedAttr.critRate || 0);
  pts += (c.advancedAttr.critDmg || 0);
  const resist = c.advancedAttr.resist || {};
  const dmg = c.advancedAttr.dmgBonus || {};
...
const Characters = {
    ...
    if (list.length === 0) { rows = '<div class="placeholder">暂无角色，点击右上角按钮添加</div>'; }
      ...
      list.forEach(c => {
        ...
        rows += `<div class="row-item" style="gap:18px;padding:22px 28px;min-height:140px;">
          ...
          <span class="row-actions" style="width:72px;justify-content:center;">
  ...
  renderDetail(id) {
    ...
    return `<div class="detail-page" data-char-id="${char.id||''}" data-is-new="${isNew}">
      <div class="toolbar"><button class="btn-primary" onclick="App.navigate('characters')">← 返回列表</button><button class="btn-primary" id="btn-save-char">保存</button>${!isNew?'<button class="btn-danger" id="btn-del-char">删除角色</button>':''}</div>
      ...
      <div class="toolbar" style="margin-top:20px;"><button class="btn-primary" id="btn-save-char2">保存</button>${!isNew?'<button class="btn-danger" id="btn-del-char2">删除角色</button>':''}</div>
    </div>`;
  },
  ...
  bindDetailEvents() {
    ...
    document.getElementById('char-realm')?.addEventListener('change', function () {
      self._updateGrowth();
      // 境界变化后刷新页面以更新元婴功法可用性
      setTimeout(() => { const d = self._collect(charId); Storage.save(STORAGE_KEY, d); App.navigate('characters/detail?id=' + (d.id || charId)); }, 0);
    });
