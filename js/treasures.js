...
function createEmptyTreasure() { return { id:'',name:'',image:'',type:'attack',subtype:'剑',grade:'下品法器',entry1:{lv1:0,lv60:0},entry2:{stat:'攻击',lv1:0,lv60:0,valType:'percent'},passiveSkills:[] }; }
...
function calcTreasureCombat(t) {
  ...
  const isFixed = (t.entry2 && t.entry2.valType === 'fixed');
  if (s==='生命') pts += isFixed ? v / 13.5 : v / 0.5;
  else if (s==='攻击') pts += isFixed ? v / 2.7 : v / 1;
  else if (s==='防御') pts += isFixed ? v / 2 : v / 2;
  else if (s==='暴击率') pts += isFixed ? v : v * 2;
  else if (s==='暴击伤害') pts += isFixed ? v : (v - 100) / 2;
  else if (s.includes('抗性')||s.includes('伤害加成')) pts += isFixed ? v : v / 0.8;
  // 特性战力加成
  (t.passiveSkills||[]).forEach(function(sk){ pts += sk.combat||0; });
...
const Treasures = {
  ...
  renderList() {
    ...
    if (filtered.length === 0) { rows = '<div class="placeholder">暂无法宝，点击右上角按钮添加</div>'; }
      ...
      filtered.forEach(t => {
        const tSrc=(t.image||'').startsWith('data:')?t.image:(t.image?'img/treasures/'+t.image:'');const imgHtml=tSrc?`<img src="${tSrc}" alt="${t.name}" class="thumb-clickable" style="cursor:pointer;">`:'<div class="row-noimg">无图</div>';
        ...
        rows += `<div class="row-item">
          ...
          ${anyAttack ? (hasEntry1 ? `<span class="row-stat" style="width:105px;">${e1Label}+${fmtNum(t.entry1.lv60)}</span>` : '<span style="width:105px;text-align:center;">—</span>') : ''}
          <span class="row-stat" style="width:100px;">${t.entry2.stat}+${fmtNum(t.entry2.lv60)}${(t.entry2.valType==='fixed')?'': '%'}</span>
          <span style="color:var(--gold);width:55px;font-size:14px;text-align:center;white-space:nowrap;">${fmtNum(calcTreasureCombat(t))}</span>
  ...
  renderDetail(id) {
    ...
    let skillsHtml = ''; (t.passiveSkills||[]).forEach((sk,i)=>{ skillsHtml += `<div class="entry-item"><div class="entry-header"><span>特性 #${i+1}</span><button class="btn-delete" data-action="del-skill" data-idx="${i}">×</button></div><input type="text" value="${sk.name||''}" data-field="tskill_name_${i}" placeholder="特性名称" style="width:100%;margin-bottom:8px;"><div style="display:flex;gap:6px;"><textarea data-field="tskill_desc_${i}" placeholder="特性描述" style="width:100%;">${sk.desc||''}</textarea><div style="flex-shrink:0;width:60px;"><label>战力</label><input type="number" value="${sk.combat||0}" data-field="tskill_combat_${i}" style="width:100%;"></div></div></div>`; });
